...
interface UsageRun { run_id: string; trace_id: string; trace_url: string | null; tenant_name: string; project_name: string; status: string; agent_role: string; model_key: string; input_tokens: number; output_tokens: number; reserved_tokens: number; budget_variance_tokens: number; budget_utilization: number; budget_status: 'within_range' | 'soft_exceeded' | 'unmeasured'; estimated_cost: number; currency: string; input_price_per_million: number; output_price_per_million: number; is_mock: boolean; created_at: string }
...
interface SupplyModel { id: string; key: string; display_name: string; provider_id: string; provider_name: string; provider_status: string; agentscope_class: string; min_tier_code: string; min_tier_name: string; input_price_per_million: number; output_price_per_million: number; context_window: number; enabled: boolean; version: number }
...
const modelContextWindow = ref(32768)
...
function openModel(model?: SupplyModel) {
  ...
  modelContextWindow.value = model?.context_window ?? 32768
...
<template>
  ...
  <div v-else v-ui-locale="locale" class="admin-shell">
    ...
    <section class="admin-content">
      ...
      <template v-else-if="activeView === 'usage'"><div v-if="usageSummary" class="kpi-grid"><article><small>输入 Tokens</small><strong>{{ formatNumber(usageSummary.input_tokens) }}</strong></article><article><small>输出 Tokens</small><strong>{{ formatNumber(usageSummary.output_tokens) }}</strong></article><article><small>总 Tokens</small><strong>{{ formatNumber(usageSummary.total_tokens) }}</strong></article><article><small>估算成本</small><strong>¥{{ usageSummary.estimated_cost.toFixed(4) }}</strong></article></div><section class="tenant-panel usage-panel"><header><div><h2>最近运行</h2><small>预算默认用于观测和软告警；价格按模型调用时的快照计算</small></div></header><div class="table-wrap"><table><thead><tr><th>租户 / 项目</th><th>Agent / 模型</th><th>Tokens</th><th>预算区间</th><th>价格快照</th><th>成本</th><th>Trace</th><th>时间</th></tr></thead><tbody><tr v-for="run in usageRuns" :key="run.run_id"><td><strong>{{ run.tenant_name }}</strong><small>{{ run.project_name }}</small></td><td><strong>{{ run.agent_role }}</strong><small>{{ run.model_key }} <span v-if="run.is_mock" class="mock-chip">Mock</span></small></td><td>{{ formatNumber(run.input_tokens + run.output_tokens) }}<small>入 {{ formatNumber(run.input_tokens) }} / 出 {{ formatNumber(run.output_tokens) }}</small></td><td><span :class="['budget-chip', run.budget_status]">{{ run.budget_status === 'soft_exceeded' ? '软超额' : run.budget_status === 'unmeasured' ? '暂无基线' : '合理区间' }}</span><small v-if="run.budget_status !== 'unmeasured'">预留 {{ formatNumber(run.reserved_tokens) }} · {{ Math.round(run.budget_utilization * 100) }}%</small></td><td>¥{{ run.input_price_per_million }} / ¥{{ run.output_price_per_million }}<small>每百万入 / 出</small></td><td>¥{{ run.estimated_cost.toFixed(6) }}</td><td><a v-if="run.trace_url" :href="run.trace_url" target="_blank" rel="noreferrer">打开 Studio</a><span v-else :title="run.trace_id">未配置 Studio</span></td><td>{{ formatDate(run.created_at) }}</td></tr></tbody></table></div></section></template>
        ...
        <section class="supply-workspace">
          ...
          <main class="model-catalog">
            ...
            <div v-if="selectedProvider" class="catalog-body">
              ...
              <div class="table-wrap model-table"><table><thead><tr><th>模型</th><th>AgentScope Class</th><th>上下文窗口</th><th>输入价 / 百万</th><th>输出价 / 百万</th><th>最低等级</th><th>版本</th><th>状态</th><th><span class="sr-only">操作</span></th></tr></thead><tbody><tr v-for="model in providerModels" :key="model.id"><td><strong>{{ model.display_name }}</strong><small>{{ model.key }}</small></td><td>{{ model.agentscope_class }}</td><td>{{ formatNumber(model.context_window) }}</td><td>¥{{ model.input_price_per_million }}</td><td>¥{{ model.output_price_per_million }}</td><td><span class="tier-chip">{{ model.min_tier_name }}</span></td><td>v{{ model.version }}</td><td><span :class="model.enabled ? 'status-connected' : 'status-suspended'">{{ model.enabled ? '启用' : '停用' }}</span></td><td><button class="table-action" @click="openModel(model)">配置</button></td></tr></tbody></table><p v-if="!providerModels.length" class="empty-state">此 Provider 下没有符合条件的模型。</p></div>
