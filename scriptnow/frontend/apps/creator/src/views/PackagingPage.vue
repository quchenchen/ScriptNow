...
const packagingVision = computed(() => pickCreativeCopy('packaging', locale.value))
const englishSpecCopy: Record<string, { platform?: string; ratio?: string; note: string }> = {
  wattpad_hd: {
    ratio: '16:25 (about 2:3)',
    note: 'High-resolution Wattpad mobile cover; scales proportionally to 512 × 800.',
  },
  wattpad_standard: {
    ratio: '16:25 (about 2:3)',
    note: 'Standard size for mobile web-fiction communities in North America and Europe.',
  },
  webnovel: {
    platform: 'Webnovel',
    note: 'Keep exactly 600 × 800 in JPG under 5 MB to avoid forced cropping.',
  },
  dreame_goodnovel: {
    note: 'Common format for international women’s-fiction platforms; can scale to 750 × 1000.',
  },
  dreame_goodnovel_hd: {
    note: 'High-resolution Dreame / GoodNovel format at the same aspect ratio.',
  },
  radish_inkitt: {
    note: 'Standard portrait format for mobile web fiction.',
  },
}
function localizedSpec(spec: OutputSpec) {
  const copy = isEnglish.value ? englishSpecCopy[spec.key] : undefined
  return {
    platform: copy?.platform ?? spec.platform,
    ratio: copy?.ratio ?? spec.ratio,
    note: copy?.note ?? spec.note,
  }
}
...
<template>
  <AppShell title="作品包装" eyebrow="书名 · 简介 · 标签 · 封面">
    ...
    <section v-if="workPackage" class="package-copy"><header><span>第 {{ workPackage.version }} 版 · {{ workPackage.language }}</span><h2 data-i18n-skip>{{ workPackage.title }}</h2></header><p data-i18n-skip>{{ workPackage.synopsis }}</p><div data-i18n-skip><span v-for="tag in workPackage.tags" :key="tag">{{ tag }}</span></div><label>调整意见<textarea v-model="feedback" placeholder="例如：更突出禁忌关系，弱化类型套路感。" /></label></section>
    <section v-if="workPackage" class="prompt-editor"><header><div><p class="eyebrow">封面 Prompt</p><h2>Agent 提炼的视觉指令</h2><p>可在生成前直接调整。最终使用版本会随封面保存，不会覆盖 Agent 原稿。</p></div><button :disabled="coverPrompt === workPackage.cover_prompt" @click="coverPrompt = workPackage.cover_prompt">恢复 Agent 原稿</button></header><textarea v-model="coverPrompt" maxlength="20000" spellcheck="false" data-i18n-skip /><footer><span :class="{ changed: coverPrompt !== workPackage.cover_prompt }">{{ coverPrompt === workPackage.cover_prompt ? '当前为 Agent 原稿' : '已由用户调整' }}</span><small>{{ coverPrompt.length }} / 20,000</small></footer></section>
    <section class="cover-config"><header><div><p class="eyebrow">输出尺寸</p><h2>选择需要生成的平台规格</h2><p>默认生成 Wattpad 高清版和 Webnovel 严格版，可自由单选或多选。</p></div></header>
      <div class="spec-grid"><button v-for="spec in specs" :key="spec.key" :class="{ selected: selected.includes(spec.key) }" @click="toggle(spec.key)"><span>{{ selected.includes(spec.key) ? '✓' : '' }}</span><strong>{{ localizedSpec(spec).platform }}</strong><b>{{ spec.width }} × {{ spec.height }} px</b><small>{{ localizedSpec(spec).ratio }} · {{ spec.formats.join('/') }}</small><p>{{ localizedSpec(spec).note }}</p></button></div>
    ...
    <section v-if="covers.length" class="cover-results"><article v-for="cover in covers" :key="cover.id"><button class="cover-delete" :class="{ confirming: deleteConfirm === cover.id }" @click.stop="deleteCover(cover.id)" :title="deleteConfirm === cover.id ? '再次点击确认删除' : '删除这张封面'">{{ deleteConfirm === cover.id ? '确认删除？' : '✕' }}</button><img :src="cover.image_url" :alt="`${cover.width} × ${cover.height} 封面候选`" /><strong>{{ specs.find((item) => item.key === cover.platform_key)?.platform }}</strong><small>{{ cover.width }} × {{ cover.height }} · {{ cover.language }}</small></article></section>
  </AppShell>
</template>
...
.cover-results article{position:relative;padding:10px;border:1px solid transparent;border-radius:14px}.cover-delete{position:absolute;top:16px;right:16px;width:28px;height:28px;border:1px solid #e2d9ca;border-radius:50%;background:#fffdf9;color:#8d3729;font-size:14px;cursor:pointer;z-index:1;box-shadow:0 2px 8px #0002}.cover-delete.confirming{width:auto;border-radius:7px;padding:3px 9px;background:#8d3729;color:#fff;font-size:11px}
...
.generation-status {
  border-color:color-mix(in srgb,var(--sf-success) 48%,var(--sf-line));
  background:var(--sf-success-soft);
  color:var(--sf-text);
}
.generation-status p,
.generation-status time {
  color:var(--sf-muted);
}
.status-spinner {
  border-color:color-mix(in srgb,var(--sf-success) 28%,var(--sf-line));
  border-top-color:var(--sf-success);
}
.package-error {
  border:1px solid color-mix(in srgb,var(--sf-danger) 42%,var(--sf-line));
  background:var(--sf-danger-soft);
  color:var(--sf-danger);
}
.package-error.actionable {
  border-color:color-mix(in srgb,var(--sf-rust) 38%,var(--sf-line));
  background:color-mix(in srgb,var(--sf-rust) 10%,var(--sf-surface));
  color:var(--sf-text);
}
.package-copy>div span {
  border:1px solid color-mix(in srgb,var(--sf-green) 40%,var(--sf-line));
  background:var(--sf-green-soft);
  color:var(--sf-text);
}
.package-copy textarea,
.prompt-editor textarea,
.prompt-editor header button,
.cover-actions select {
  border-color:var(--sf-line);
  background:var(--sf-surface-raised);
  color:var(--sf-text);
}
.package-copy textarea::placeholder,
.prompt-editor textarea::placeholder {
  color:var(--sf-muted);
}
.prompt-editor textarea {
  background:var(--sf-surface-muted);
  caret-color:var(--sf-rust);
}
.prompt-editor header button:disabled {
  color:var(--sf-muted);
  opacity:.62;
}
.prompt-editor footer .changed,
.package-copy header span,
.spec-grid small {
  color:var(--sf-rust);
}
.spec-grid button {
...
.spec-grid button:hover {
  border-color:color-mix(in srgb,var(--sf-green) 42%,var(--sf-line));
  background:color-mix(in srgb,var(--sf-green) 8%,var(--sf-surface-raised));
}
.spec-grid button.selected {
  border-color:var(--sf-green);
  background:var(--sf-green-soft);
  box-shadow:inset 0 0 0 1px var(--sf-green);
}
.spec-grid button>span {
  color:var(--sf-green);
}
.cover-results article {
  border-color:var(--sf-line);
  background:var(--sf-surface);
  color:var(--sf-text);
}
...
.cover-delete {
  border-color:color-mix(in srgb,var(--sf-rust) 45%,var(--sf-line));
  background:var(--sf-surface-raised);
  color:var(--sf-rust);
  box-shadow:var(--sf-shadow);
}
.cover-delete.confirming {
