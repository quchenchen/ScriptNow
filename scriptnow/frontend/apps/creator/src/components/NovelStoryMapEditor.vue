...
const validDraft = computed(() => draft.value.some((volume) => volume.chapters.length > 0)
  && draft.value.every((volume) => volume.chapters.every((chapter) =>
    chapter.title.trim()
    && Number.isFinite(chapter.target_words)
    && chapter.target_words > 0
    && Boolean(chapter.point_of_view?.trim()))))
...
function add(volume: Volume) {
  ...
  const previous = volume.chapters.at(-1)
  volume.chapters.push({
    id: `chapter-${suffix}`,
    ordinal,
    title: `新章节 ${ordinal}`,
    target_words: previous?.target_words ?? Number.NaN,
    point_of_view: previous?.point_of_view ?? '',
    beats: [],
  })
}
</script>
...
<template>
  <section class="structure-editor" aria-label="编辑 Novel StoryMap">
    ...
    <footer><span>保存后形成 Novel 结构候选，不会直接修改正文。新增章节需明确标题、篇幅与视角。</span><button class="primary" :disabled="!validDraft" @click="emit('save', draft)">保存为结构候选</button></footer>
  </section>
</template>
