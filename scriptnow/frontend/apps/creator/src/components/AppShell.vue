...
<template>
  <div class="creator-shell" :class="{ 'sidebar-hidden': layout.sidebarHidden, 'writer-focus': layout.writerFocus, dragging }" :style="shellStyle">
    <aside class="creator-sidebar" :class="{ open: layout.mobileOpen }">
      ...
      <nav v-else class="shell-nav"><p>{{ t('creator.workspace') }}</p><RouterLink to="/">◈ {{ t('creator.dashboard') }}</RouterLink></nav>
      <RouterLink v-else to="/new" class="create-project-btn">✦ {{ t('creator.create') }}</RouterLink>
