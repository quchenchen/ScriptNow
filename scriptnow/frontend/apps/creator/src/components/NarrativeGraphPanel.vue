...
type GraphNode = { id: string; type: NarrativeNodeType | string; label: string; summary: string; chapters?: string[]; unit_count?: number; evidence?: Evidence[]; evidence_count?: number }
type GraphEdge = { id: string; type: NarrativeRelationType | string; source: string; target: string; label: string; inference: boolean }
type NormalizedGraphNode = Omit<GraphNode, 'type'> & { type: NarrativeNodeType }
type NormalizedGraphEdge = Omit<GraphEdge, 'type'> & { type: NarrativeRelationType }
...
const normalizedNodes = computed<NormalizedGraphNode[]>(() => (graph.value?.nodes ?? []).map((node) => {
  const type = canonicalNarrativeNodeType(node.type)
  return { ...node, type: type ?? 'concept' }
}))
const normalizedEdges = computed<NormalizedGraphEdge[]>(() => {
  const nodeIds = new Set(normalizedNodes.value.map((node) => node.id))
  return (graph.value?.edges ?? []).flatMap((edge) => {
    if (!edge?.id || !nodeIds.has(edge.source) || !nodeIds.has(edge.target)) return []
    return [{ ...edge, type: canonicalNarrativeRelationType(edge.type) ?? 'affiliation' }]
  })
})
...
const connected = computed(() => {
  ...
  return normalizedEdges.value.filter((edge) => edge.source === selected.value?.id || edge.target === selected.value?.id).map((edge) => ({ edge, node: byId.get(edge.source === selected.value?.id ? edge.target : edge.source) }))
})
const focusedNodeIds = computed(() => {
  ...
  const byId = new Map(normalizedNodes.value.map((node) => [node.id, node]))
  const neighbors = normalizedEdges.value.flatMap((edge) => {
...
const canvasNodes = computed<Node[]>(() => {
  ...
  for (const edge of normalizedEdges.value) if (focusedNodeIds.value.has(edge.source) && focusedNodeIds.value.has(edge.target)) layout.setEdge(edge.source, edge.target)
  dagre.layout(layout)
...
const canvasEdges = computed<Edge[]>(() => normalizedEdges.value
...
<template>
  <section class="narrative-graph-workspace" :class="{ 'graph-expanded': expanded }">
    ...
    <div v-else-if="graph?.status === 'not_built'" class="graph-empty"><strong>{{ graphMode === 'creative' ? ui('创作图谱尚未建立', 'No creative graph yet') : ui('尚未建立素材图谱', 'No source graph yet') }}</strong><span>{{ graphMode === 'creative' ? ui('采纳章节后将自动提取人物与事件关系，下次打开即可查看。', 'Characters and events are extracted automatically when chapters are adopted.') : ui('上传或解析小说素材后，这里会出现章节、人物与事件之间的联系。', 'Upload or parse a novel manuscript to reveal chapter, character, and event connections here.') }}</span></div>
