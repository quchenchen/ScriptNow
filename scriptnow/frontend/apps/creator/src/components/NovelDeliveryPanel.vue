...
<template>
  <div class="delivery-toolbar"><span>Novel · {{ selectedCount ? `${selectedCount} 章已就绪` : '交付工具' }}</span><button @click="router.push('/')">回到控制台</button><button @click="open('history')">历史版本</button><button class="primary" @click="open('export')">导出小说</button></div>
