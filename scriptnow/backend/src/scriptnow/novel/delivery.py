...
class NovelExportService:
    def __init__(
        self,
        database: Database,
        renderer: Callable[..., bytes] = render_novel_docx,
    ...
    async def export(
        ...
        *,
        ...
        if translation_mode not in {"none", "faithful"}:
            raise NovelDeliveryError("归化翻译敬请期待，当前仅支持常规翻译")
        if translation_mode == "faithful" and not target_language:
            raise NovelDeliveryError("常规翻译需要选择目标语言")
        async with self.database.session() as session:
            project = await _project(session, tenant_id, project_id)
            existing = (
                await session.scalars(
            ...
            if translation_mode == "faithful":
                if self.translator is None:
                    raise NovelDeliveryError("translation service is unavailable")
                try:
                    translated = await self.translator.translate(
                        ...
                        units=tuple(
                            TranslationUnit(
                                ...
                                blocks=tuple(block.model_dump(mode="json") for block in chapter.blocks),
                            )
                    ...
                    raise NovelDeliveryError(str(error)) from error
                chapters = [
                    NovelExportChapter(
                        volume_title=unit.titles["volume_title"],
                        chapter_title=unit.titles["chapter_title"],
                        blocks=tuple(NovelBlock.model_validate(block) for block in unit.blocks),
                    )
