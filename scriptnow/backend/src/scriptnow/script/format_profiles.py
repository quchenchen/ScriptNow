...
2. action 是单个可拍画面；仅关键分镜（特写、闪回、动作节拍）用“▲”或“▲【…】”开头标记，常规画面描述不加▲，严禁每个段落都加▲。示例：常规“她攥紧账本，指尖发白”；关键“▲【特写】账本缺页的一角”。
3. 每段 dialogue 前必须紧邻 character；character 写“人物（情绪/动作）”，如“林昭宁（OS，迷糊）”。OS=内心独白，VO=画外旁白；旁白每 25-35 个分镜才出现一次，单段不超过 30 字。
4. 每个分镜组画面描述控制在 20-30 字以内；台词必须是自然连贯的完整句子：用逗号/顿号组织成一条完整台词，严禁电报式短句堆叠（如“坐下。从第一页看。三年前。我做的。”应写成“坐下，从第一页看——三年前宋家尽调是我做的。”）；列举信息写成一条连贯台词；内心读白用 OS，旁白用 VO；单条台词长度不限。
