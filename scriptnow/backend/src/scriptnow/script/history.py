...
from scriptnow.platform.models import ProjectEventModel, ProjectModel, ProjectSnapshotModel
from scriptnow.script.contracts import ScriptBlock
from scriptnow.script.domain import (
    RevisionStatus,
    ScriptDocumentRevisionModel,
    ScriptSnapshotContentModel,
)


class ScriptHistoryError(RuntimeError):
    pass


class ScriptHistoryConflict(ScriptHistoryError):
    pass
...
class ScriptHistoryService:
    ...
    async def create_snapshot(
        ...
        if not name.strip():
            raise ScriptHistoryError("snapshot name is required")
        async with self.database.session() as session:
            await _project(session, tenant_id, project_id)
            documents = await _current_documents(session, project_id)
            if not documents:
                raise ScriptHistoryError("snapshot requires adopted Script scenes")
            version = (
                int(
            ...
            await session.flush()
            session.add(ScriptSnapshotContentModel(snapshot_id=snapshot.id, documents=documents))
            await _event(
                session,
                tenant_id,
                project_id,
                event_key=f"script:snapshot:create:{snapshot.id}",
                action="script_snapshot.created",
        ...
        async with self.database.session() as session:
            ...
            return list(
                await session.scalars(
                    ...
                    .where(
                        ProjectSnapshotModel.project_id == project_id,
                        ProjectSnapshotModel.medium == "script",
                    )
                    .order_by(ProjectSnapshotModel.version.desc())
                )
            )
    ...
    async def rollback(
        ...
        *,
        ...
        idempotency_key: str,
    ) -> list[ScriptDocumentRevisionModel]:
        async with self.database.session() as session:
            snapshot, content = await _snapshot(session, tenant_id, project_id, snapshot_id)
            replay_keys = [
            ...
            existing = list(
                await session.scalars(
                    select(ScriptDocumentRevisionModel).where(
                        ...
                        ScriptDocumentRevisionModel.idempotency_key.in_(replay_keys),
                    )
                )
            )
            if existing:
            ...
            if _hash(current) != expected_current_hash:
                raise ScriptHistoryConflict("Script changed after rollback preview")
            ...
            for document in content.documents:
                ...
                blocks = [
                    ScriptBlock.model_validate(item).model_dump(mode="json")
                    for item in document["blocks"]
                ]
                current_document = current_by_unit.get(unit_id)
                if current_document:
                    adopted = await session.get(
                        ScriptDocumentRevisionModel, str(current_document["revision_id"])
                    )
                    if adopted:
                ...
                number = (
                    int(
                        await session.scalar(
                            select(
                                func.coalesce(
                                    func.max(ScriptDocumentRevisionModel.revision_number), 0
                                )
                            ).where(
                                ScriptDocumentRevisionModel.project_id == project_id,
                                ScriptDocumentRevisionModel.scene_id == unit_id,
                            )
                        )
                        or 0
                    )
                ...
                revision = ScriptDocumentRevisionModel(
                    project_id=project_id,
                    scene_id=unit_id,
            ...
            await _event(
                ...
                tenant_id,
                project_id,
                event_key=f"script:snapshot:rollback:{idempotency_key}",
                action="script_snapshot.rolled_back",
                aggregate_id=snapshot.id,
...
async def _project(session, tenant_id: str, project_id: str) -> ProjectModel:
    ...
    if project is None or project.tenant_id != tenant_id or project.medium != "script":
        raise ScriptHistoryError("Script project is outside tenant scope")
    return project
...
async def _snapshot(session, tenant_id, project_id, snapshot_id):
    ...
    content = await session.get(ScriptSnapshotContentModel, snapshot_id)
    if (
        snapshot is None
        or snapshot.project_id != project_id
        or snapshot.medium != "script"
        or content is None
    ):
        raise ScriptHistoryError("Script snapshot is unavailable")
    return snapshot, content


async def _current_documents(session, project_id: str, scope: set[str] | None = None):
    query = select(ScriptDocumentRevisionModel).where(
        ScriptDocumentRevisionModel.project_id == project_id,
        ScriptDocumentRevisionModel.status == RevisionStatus.ADOPTED,
    )
    if scope is not None:
        query = query.where(ScriptDocumentRevisionModel.scene_id.in_(scope))
    records = list(await session.scalars(query.order_by(ScriptDocumentRevisionModel.scene_id)))
    return [
        {"unit_id": item.scene_id, "revision_id": item.id, "blocks": item.blocks}
        for item in records
    ]
...
async def _event(
    ...
    session.add(
        ProjectEventModel(
            ...
            aggregate={"type": "script_snapshot", "id": aggregate_id},
