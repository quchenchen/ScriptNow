...
from scriptnow.platform.narrative_graph_extractor import NarrativeGraphExtractor
...
def _readable_node_rows(
    stored_nodes: list[object], units_by_id: dict[str, object]
) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for node in stored_nodes:
        try:
            evidence_unit_ids = (
                list(node.evidence_unit_ids)
                if isinstance(node.evidence_unit_ids, list | tuple)
                else []
            )
            rows.append(
                {
                    "id": node.id,
                    "type": compatible_node_type(node.node_type).value,
                    "label": node.name,
...
def _readable_edge_rows(
    stored_edges: list[object], readable_node_ids: set[str]
) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for edge in stored_edges:
        try:
            if (
                edge.source_node_id not in readable_node_ids
                or edge.target_node_id not in readable_node_ids
            ):
                continue
            rows.append(
                {
                    "id": edge.id,
                    "type": compatible_relation_type(edge.edge_type).value,
                    "source": edge.source_node_id,
                    "target": edge.target_node_id,
                    "label": edge.description,
                    "inference": bool(edge.inference),
                }
            )
        except (AttributeError, TypeError, ValueError):
            continue
...
def create_narrative_graph_router(
    ...
    async def graph(
        ...
        node_rows = _readable_node_rows(stored_nodes, units_by_id)
        if not node_rows:
            node_rows = list(chapters.values())
        readable_node_ids = {row["id"] for row in node_rows}
        edge_rows = _readable_edge_rows(stored_edges, readable_node_ids)
        if not edge_rows:
