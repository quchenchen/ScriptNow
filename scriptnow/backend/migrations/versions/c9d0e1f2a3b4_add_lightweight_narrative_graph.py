...
def upgrade() -> None:
    # Early development builds created these tables through metadata.create_all
    # before this migration was added. Treat a complete runtime-created set as
    # already migrated so Alembic can repair its version ledger safely.
    ...
    if "narrative_indexes" in existing:
        missing = graph_tables - existing
        if missing:
            raise RuntimeError(
                f"partial narrative graph schema requires manual repair: {sorted(missing)}"
            )
        return
    op.create_table(
        "narrative_indexes",
