"""Memory Service — Character + Foreshadow + Scene reads (SQLAlchemy).

Character/Foreshadow write paths accept plain dicts (from the API layer) and
translate them onto ORM objects.

Scene read paths read the ``scenes`` table directly (issue #06 promoted it out
of ``episodes.scenes`` JSON).
"""
...
from app.models import Character, Episode, Foreshadow, Scene
...
class MemoryService:
    ...
    async def list_characters(self, project_id: int) -> list[dict]:
        r = await self.db.execute(
            ...
            .order_by(Character.role, Character.first_appearance)
        )
    ...
    async def add_character(self, project_id: int, data: dict) -> int:
        c = Character(
            ...
            is_organization=data.get("is_organization", 0),
    ...
    async def delete_character(self, project_id: int, char_id: int) -> None:
        await self.db.execute(
            update(Character)
            .where(Character.id == char_id, Character.project_id == project_id)
            .values(status="deceased")
        )
        await self.db.commit()
    ...
    async def add_foreshadow(self, project_id: int, data: dict) -> int:
        f = Foreshadow(
            ...
            subtlety=data.get("subtlety", 5),
            is_long_term=data.get("is_long_term", 0),
            plant_episode=data.get("plant_episode"),
    ...
    async def update_foreshadow(self, project_id: int, fid: int, data: dict) -> bool:
        stmt = update(Foreshadow).where(
            Foreshadow.id == fid, Foreshadow.project_id == project_id
        )
    ...
    async def list_scenes(self, project_id: int) -> list[dict]:
        """Return per-location scene counts across the project.

        Reads directly from the ``scenes`` table joined to ``episodes``. Only
        counts non-empty locations.
        """
        stmt = (
            select(Scene.location, func.count(Scene.id).label("cnt"))
            .join(Episode, Scene.episode_id == Episode.id)
            .where(Episode.project_id == project_id)
            .where(Scene.location != "")
            .group_by(Scene.location)
        )
        r = await self.db.execute(stmt)
        return [{"name": row.location, "count": row.cnt} for row in r.all()]
