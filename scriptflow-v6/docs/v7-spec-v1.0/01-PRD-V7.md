...
| 取代关系 | 本文档取代 `PLAN-V7-REVISION-FOCUS.md` 中的分期与范围定义；该文档保留作背景分析 |
...
ScriptFlow 是 **AI Agent 团队驱动的剧本/小说创作平台**。核心隐喻是 **Growing（生长）**：用户不是在"填表生成内容"，而是与一支人格化的 Agent 团队（创意导演、架构规划师、写作者、审读编辑）协作，让作品从创意种子逐步生长为可交付的剧本。
...
**TR-2.3.2 候选不变式（继承 V6 铁律）**：所有 `propose_*` 工具写入候选表并返回候选 ID，**永不直写**项目真理表。采纳（Adopt）永远是用户通过产品 API 完成。
...
**TR-2.5.3 唯一事实源**：修订时间线、历史版本列表、导出记录、决策流视图**必须**实现为 `project_events` 的过滤投影，禁止各自建表重复记录。用户消息（含选区引用 quote）同样入流。
...
### 2.11 模型路由与等级池（ADR-0003，修订 ADR-0002）
...
- **FR-3.0.4 线性门控（继承 V6）**：阶段由项目状态推导（未采纳 StoryCore → 发散；未规划 → 蓝图；已规划 → 写作），未达阶段的导航项禁用并给出原因提示。
...
- **FR-3.5.7** 所有蓝图内容由 Architect 生成为候选 → 用户采纳落库（V6 既有 API）；蓝图实体是修订锚点的引用目标（FR-3.8）。改编项目生成时经 RAG 引用原著。

### 3.6 StoryMap（集×场目录）

- **FR-3.6.1** 树形：集头（编号/标题/所属弧线标签/场数）展开场行（场号/标题/时长目标）；汇总条（总集数/总场数/按弧线分布）。
- **FR-3.6.2** 点击场行 → 跳转 Writer 并定位该场。结构调整（增删/重排）沿用 V6 plan/impact 确认流。

### 3.7 逐场写作（Writer 三栏）

- **FR-3.7.1 左栏 场景目录**:本集场次列表，状态点 done(完稿)/writing(写作中)/draft(草稿)，点击切换当前场。
- **FR-3.7.2 中栏 编辑器**：场景头（集·场号/标题/时长目标/状态/**格式徽章-锁定**）+ 结构化正文段落。**正文存储为结构化段落数组**（`{type: slugline|action|character|dialogue|transition, text, para_id}`），这是格式渲染、修订定位、DOCX 导出的共同地基。中国/好莱坞两种 CSS 渲染（好莱坞：Courier、居中角色名、缩进对白）。
- **FR-3.7.3 生成与采纳**：写作者按 context pack + 格式技能产出场景候选 → Dock 对话流中采纳/反馈修订（继承 V6 revise 循环）；采纳后场景状态推进并事件留痕。
- **FR-3.7.4 右栏双 Tab**：📋 上下文（当前角色卡/伏笔追踪/场景设定/情感标记——来自蓝图与连续性账本的真实数据）｜📝 修订痕迹（badge=未处理 Finding 数）。
...
```
id · project_id · unit_id(场) · para_id(段落锚) · domain(worldview|character|arc|event|foreshadow)
...
original_excerpt + locator(段落内偏移) · diagnosis · suggestion · confidence(high|mid|low)
status(open|accepted|dismissed|stale) · created_at · decided_at · trace_id
```
...
- **FR-3.8.4 采纳语义**：suggestion 以段落 patch 应用 → 文稿新版本（V6 版本化机制）→ Finding=accepted → `decision` 事件；涉及蓝图实体状态变化时联动 story_bible_changes/cascade 通道。**忽略**：status=dismissed，留痕不删除。正文再编辑导致 locator 失效的 open Finding → `stale`。
...
- **FR-3.9.2** 发送（可附加指令）→ 用户消息连同 quote 入流 → 路由至写作者 `selection-edit` 技能（V6 ai-edits 五模式复用，"修订"模式改路由至审读编辑产出 Finding）→ 流式返回替换候选（diff 视图）→ 对话流内采纳/继续反馈。
...
| # | 规则 |
|---|---|
| BR-1 | **候选不变式**：Agent 一切写意图产出候选；采纳是用户专属动作；采纳/拒绝/过期全程可追溯 |
| BR-2 | **事件流唯一事实源**：修订时间线/历史版本/导出记录/决策流=`project_events` 投影，禁止平行记录 |
...
**账务域**：`tenants`（V6 users 扩展：tier/status/joined_at）· `subscriptions`（tier/period/monthly_quota/status）· `orders`（type=subscribe|pack|grant/amount/tokens/status）· `credit_ledger`（type/tier_scope/delta/balance_after/ref_order）· `token_usage`（trace_id/tenant/project/agent_role/model_key/in/out/cost_est/created_at）
...
- **安全**：JWT（创作者/管理员分域）；Provider Key AES 加密存储不回显；管理 API 全审计；MCP/Bash 默认沙箱+确认；CORS 白名单。
...
| Phase | 范围 | 关键验收 |
|---|---|---|
| **P0 定案与地基** | 决策点批复；ADR-0003(等级模型池)/0004(事件总线)/0005(AgentScope 底座)；前端双 SPA 脚手架与组件化架构；Studio/DockerWorkspace 可用性验证 | ADR 合入；`npm run build` ×2 绿 |
| **P1 AgentScope 底座重构** | AgentFactory；领域工具四组；技能迁移 LocalSkillLoader；事件桥+SSE；计量/预算/记忆/权限中间件；agent_states 持久化；供给域表+seed 配置（无 UI） | 四角色经新底座跑通 V6 全链路；usage/events 入库；pytest 绿 |
...
| **P6 格式+导出+版本** | 结构化正文契约落地/双格式渲染/DOCX/快照 diff 回滚 | Word/Pages 排版正确；回滚可逆 |
...
*附录 B（术语表）见 §1.3；附录 C（V6 复用清单）见 PLAN-V7-REVISION-FOCUS.md §二。*
