...
| archive | 仅有历史、研究或设计参考价值 | 移入 `docs/archive/<date>/` 并标记非开发依据 |
...
- 现行规格、ADR 和 AgentScope 验证证据继续保留在 `docs/v7-spec-v1.1/`。
- 阶段性研究网页、融合调研、实现审计和截图已移入 `docs/archive/2026-07-29/`。
- 根目录上传素材已移出 Git 开发树，保存在本机 `.local/archives/2026-07-29/source-materials/`。
- 六个旧名称或重置前数据库已移出运行目录，保存在
  `.local/archives/2026-07-29/runtime-databases/`；当前 `scriptnow.db` 作为本机运行状态
  继续被 `.gitignore` 隔离，不属于版本内容。
- 可重建缓存、日志和系统元数据不构成归档资产，清理后由工具按需重建。
- `docs/archive/` 不构成当前需求、接口或验收依据；如需复用，必须重新执行契约对照。
