...
class BackupService:
    ...
    def create(self, *, database_path: Path, workspace_root: Path) -> BackupResult:
        ...
        with tempfile.TemporaryDirectory(prefix="scriptflow-backup-") as temporary:
    ...
    def restore(
        ...
        *,
        ...
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            ...
            staging = Path(
                tempfile.mkdtemp(prefix="scriptflow-restore-", dir=target_database.parent)
            )
            try:
    ...
    def _validate(self, archive: zipfile.ZipFile) -> dict[str, object]:
        ...
        if manifest.get("format") != "scriptflow-v7-backup" or manifest.get("version") != 1:
