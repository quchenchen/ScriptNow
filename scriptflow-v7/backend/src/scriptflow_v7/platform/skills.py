...
class SkillCatalog:
    ...
    def _read(self, path: Path) -> SkillDescriptor:
        ...
        scriptflow_metadata = metadata.get("scriptflow", {}) if isinstance(metadata, dict) else {}

        def product_value(key: str, default: object = None) -> object:
            # Keep legacy top-level metadata readable while new skills remain
            # portable and valid under the common Agent Skills schema.
            return scriptflow_metadata.get(key, post.get(key, default))
