...
class AgentRuntime:
        ...
        try:
            ...
            agent = Agent(
                ...
                react_config=ReActConfig(
                    # Development runs intentionally get the full safety cap:
                    # long-form writing may need several skill/tool turns before
                    # producing the final chapter. Production remains governed
                    # by the published agent policy and billing reservation.
                    max_iters=(
                        12
                        if self.settings.environment != "production"
                        else min(
                            int(dict(values.get("policy") or {}).get("max_iters", 6)),
                            12,
                        )
                    )
                    ),
    ...
    def _system_prompt(role: str, soul: str, *, language: str = "zh-CN") -> str:
        ...
        return (
            f"你是 ScriptFlow 的 {role} Agent。{responsibility}\n"
