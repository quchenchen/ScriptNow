...
async def seed() -> None:
    ...
    async with database.session() as session:
        user = (
            await session.scalars(select(UserModel).where(UserModel.email == "creator@scriptflow.local"))
        ).one_or_none()
    if user is None:
        tenant, _ = await auth.create_tenant_owner(
            tenant_name="Local Studio",
            email="creator@scriptflow.local",
            password="scriptflow-local-password",
        )
    async with database.session() as session:
        user = (
            await session.scalars(
                select(UserModel).where(UserModel.email == "creator@scriptflow.local")
            )
        ).one()
        user.is_admin = True
    ...
    print("Seeded creator@scriptflow.local / scriptflow-local-password")
