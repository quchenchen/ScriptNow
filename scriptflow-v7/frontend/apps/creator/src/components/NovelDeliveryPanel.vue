...
async function rollback() {
  ...
  busy.value = true
  try { await api(`/novel/projects/${props.projectId}/snapshots/${activeSnapshot.value.id}/rollback`, { method: 'POST', body: JSON.stringify({ expected_current_hash: preview.value.current_hash, idempotency_key: crypto.randomUUID() }) }); mode.value = undefined; window.dispatchEvent(new CustomEvent('scriptflow:document-changed')) }
