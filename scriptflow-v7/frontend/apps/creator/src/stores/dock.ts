...
const DOCK_WIDTH_KEY = 'scriptflow-ui-agent-dock-width'
const DOCK_HEIGHT_KEY = 'scriptflow-ui-agent-dock-height'
...
export const useDockStore = defineStore('agent-dock', {
  ...
  actions: {
    ...
    async adoptProposal(projectId: string) {
      ...
      await this.load(projectId)
      window.dispatchEvent(new CustomEvent('scriptflow:document-changed'))
    },
    continueProposal() {
      if (!this.proposal) return
      this.quote = {
