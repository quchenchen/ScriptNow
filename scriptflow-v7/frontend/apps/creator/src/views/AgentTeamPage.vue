  ...
  <AppShell title="创作团队" eyebrow="项目角色配置">
    ...
    <div class="agent-team-grid">
      <article v-for="member in team.members" :key="member.role_key" class="agent-config-card">
        ...
        <label>运行模型<select v-model="member.model_id"><option v-for="model in availableModels" :key="model.id" :value="model.id">{{ model.display_name }} · {{ model.provider_name }}</option></select></label>
