...
<template>
  <main v-if="!session" class="admin-login">
    <section><span>S</span><p class="eyebrow">ScriptFlow Admin</p><h1>{{ t('admin.hero') }}</h1><p>{{ t('admin.heroDescription') }}</p></section>
    <form @submit.prevent="login"><button class="admin-locale-switch" type="button" :aria-label="t('locale.label')" @click="toggleLocale">{{ t('locale.action') }}</button><p class="eyebrow">{{ t('admin.loginEyebrow') }}</p><h2>{{ t('admin.loginTitle') }}</h2><label>{{ t('common.email') }}<input v-model="email" type="email" required /></label><label>{{ t('common.password') }}<input v-model="password" type="password" minlength="12" required /></label><p v-if="error" class="error">{{ error }}</p><button :disabled="busy">{{ busy ? t('admin.loggingIn') : t('admin.login') }}</button></form>
  </main>
  <div v-else class="admin-shell">
    <aside><a href="/" class="admin-brand"><b>S</b><span>ScriptFlow V7<small>Admin Console</small></span></a><nav><p>{{ t('admin.operations') }}</p><button :class="{ active: activeView === 'tenants' }" @click="activeView = 'tenants'"><PhBuildings />{{ t('admin.tenants') }}</button><button :class="{ active: activeView === 'usage' }" @click="activeView = 'usage'"><PhChartLine />{{ t('admin.usage') }}</button><p>{{ t('admin.supply') }}</p><button :class="{ active: activeView === 'supply' }" @click="activeView = 'supply'"><PhStack />Provider / Model</button><button :class="{ active: activeView === 'capabilities' }" @click="activeView = 'capabilities'"><PhRobot />{{ t('admin.capabilities') }}</button><button :class="{ active: activeView === 'mcp' }" @click="activeView = 'mcp'"><PhPlugsConnected />{{ t('admin.mcp') }}</button><button :class="{ active: activeView === 'memory' }" @click="activeView = 'memory'"><PhBrain />{{ t('admin.memory') }}</button></nav><button class="admin-logout" @click="logout"><PhSignOut />{{ t('admin.logout') }}</button></aside>
    <section class="admin-content">
      <header><div><p class="eyebrow">Platform Operations</p><h1>{{ viewTitle }}</h1></div><div class="admin-header-actions"><span>{{ viewHint }}</span><button class="admin-locale-switch" :aria-label="t('locale.label')" @click="toggleLocale">{{ t('locale.action') }}</button></div></header>
      ...
      <template v-else-if="activeView === 'capabilities'">
        ...
        <section class="agent-capability-layout">
          ...
          <main class="skill-governance">
            ...
            <section class="skill-plan-audit"><header><div><p class="eyebrow">Runtime Explainability</p><h2>最近 SkillPlan</h2><small>每次运行固定 CreativeProfile 与选用 Skill；后续修改技能不会悄悄改变历史运行。</small></div></header><div v-if="runtimeSkillPlans.length" class="skill-plan-list"><article v-for="item in runtimeSkillPlans" :key="item.run_id"><header><div><strong>{{ item.project_name }}</strong><small>{{ new Date(item.created_at).toLocaleString('zh-CN') }} · Run {{ item.run_id.slice(0, 8) }}</small></div><span>{{ item.plan.medium }} · {{ item.plan.role_key }} · {{ item.plan.stage }}</span></header><div class="skill-plan-profile"><span v-for="genre in item.plan.creative_profile.genres" :key="`plan-genre-${genre}`">题材 {{ genre }}</span><span v-for="theme in item.plan.creative_profile.themes" :key="`plan-theme-${theme}`">主题 {{ theme }}</span><span v-for="style in item.plan.creative_profile.styles" :key="`plan-style-${style}`">风格 {{ style }}</span><span v-for="structure in item.plan.creative_profile.structures" :key="`plan-structure-${structure}`">结构 {{ structure }}</span></div><ol><li v-for="selection in item.plan.selections" :key="selection.key"><div><strong>{{ selection.key }}</strong><span>{{ selection.layer }} · {{ selection.score }} 分</span></div><p>{{ selection.reasons.join('；') }}</p></li></ol></article></div><p v-else class="empty-state">新运行将在这里显示真实 SkillPlan；已有历史运行若未保存该快照，不会补造解释数据。</p></section>
          </main>
        </section>
      </template>
      ...
      <template v-else>
        <section class="governance-stack memory-view">
          ...
          <article class="tenant-panel"><header><div><p class="eyebrow">Long-term Memory</p><h2>内容浏览与治理</h2></div><small>{{ memoryEntries.length }} 条</small></header><div class="memory-list"><article v-for="entry in memoryEntries" :key="entry.id"><header><div><strong>{{ entry.project_name }} · {{ entry.role_key }}</strong><small>{{ entry.tenant_name }} · {{ new Date(entry.updated_at).toLocaleString('zh-CN') }}</small></div><span>{{ entry.content_hash.slice(0, 8) }}</span></header><p>{{ entry.content }}</p><footer><button @click="editMemory(entry, 'correct')">纠偏</button><button @click="editMemory(entry, 'compress')">压缩</button><button class="danger" @click="deleteMemory(entry)">删除</button></footer></article><p v-if="!memoryEntries.length" class="empty-state">当前没有长期记忆。</p></div></article>
          <article class="tenant-panel"><header><div><p class="eyebrow">Append-only Audit</p><h2>记忆操作审计</h2></div></header><div class="audit-stream"><p v-for="item in memoryAudit" :key="item.id"><span>{{ item.operation }}</span><strong>{{ item.memory_entry_id.slice(0, 8) }}</strong><small>{{ new Date(item.created_at).toLocaleString('zh-CN') }}</small></p></div></article>
        </section>
      </template>
    </section>
